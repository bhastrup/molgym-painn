...
class PainnEquivariantAC(AbstractActorCritic):
    def __init__(
        self,
        ...
        self.phi_equivariant = MLP(
            input_dim=(self.num_afeats * 3),
    ...
    def to_action_space(self, action: torch.Tensor, observation: ObservationType) -> ActionType:
        """This function is responsible for creating an ActionSpace.
        
        This function actually turns the chosen actions into an atom-position tuple.
        """
        stop, focus, element, distance, x, y, z = to_numpy(action)

        if stop:
            return self.action_space.build(ase.Atoms())

        direction = np.hstack((x, y, z))
        ...
        position = determine_coordinates(positions=positions,
                                         focus=focus,
                                         distance=distance,
                                         direction=direction)
    ...
    def make_atomic_tensors(
        ...
        features = torch.zeros(size=(len(observations), self.num_atoms, self.num_afeats),
                               dtype=torch.float32,
                               device=self.device)
        vector_features = torch.zeros(size=(len(observations), self.num_atoms, 3, self.num_afeats),
                                      dtype=torch.float32,
                                      device=self.device)
        ...
        for i, obs in enumerate(observations):
            ...
            bag_tuple = [count for _, count in formula]
            if len(atoms) > 0:
            ...
            action_mask[i, 3] = len(atoms) >= 2  # direction_x
            action_mask[i, 4] = len(atoms) >= 2  # direction_y
            action_mask[i, 5] = len(atoms) >= 2  # direction_z
        ...
        for i, obs in enumerate(observations):
            ...
            if len(atoms) > 0:
                ...
                features[i, :len(atoms), :] = nodes_scalar[slc, :]
                vector_features[i, :len(atoms), :, :] = nodes_vector[slc, :, :]

        return (
            features,  # n_obs x n_atoms x n_afeats
            vector_features,  # n_obs x n_atoms x 3 x n_afeats
    ...
    def step(self, observations: List[ObservationType], actions: Optional[np.ndarray] = None) -> dict:
        ...
        # atomic_vector_feats: n_obs x n_atoms x 3 x n_afeats
        # focus_mask: n_obs x n_atoms
        # focus_mask_next: n_obs x n_atoms
        # element_count: n_obs x n_zs
        # action_mask: n_obs x 6

        atomic_feats, atomic_vector_feats, focus_mask, focus_mask_next, element_count, action_mask = self.make_atomic_tensors(observations)
        ...
        # Direction
        # Non-zero vectors for those that have at least 2 atoms
        focused_vector = (atomic_vector_feats.flatten(2).transpose(1, 2) @ focus_oh[:, :, None]).squeeze(-1)
        direction_mean = self.phi_equivariant(focused_vector)
        normal_direction_mean = torch.nn.functional.normalize(direction_mean, p=2, dim=1)

        direction_dist = torch.distributions.Normal(normal_direction_mean, torch.exp(1e-6 + self.log_stds[1]).unsqueeze(-1))
        ...
        distance_mean = torch.tanh(self.phi_dist(f))
        ...
        if actions is None:
            actions = torch.cat(
                [stop, focus.float(), element.float(), distance, direction], dim=-1)
