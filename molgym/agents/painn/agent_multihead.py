...
class PainnACMulti(AbstractActorCritic):
    def __init__(
        self,
        ...
        self.encoder_layer = nn.TransformerEncoderLayer(d_model=self.num_afeats, nhead=8)
        self.transformer_encoder = nn.TransformerEncoder(self.encoder_layer, num_layers=6)
        self.src = torch.rand(32, self.num_atoms, self.num_afeats)
        masker= data_painn.pad_and_stacker(self.num_atoms, self.num_zs)
        self.out = self.transformer_encoder(self.src, mask=masker)
        self.to(device)
    ...
    def step(self, observations: List[ObservationType], actions: Optional[np.ndarray] = None) -> dict:
        ...
        #print(f'Hey it is I {features}')

        print(f"atomic_feats shape: " + str(atomic_feats.shape))
        #print(atomic_feats)
        #exit()
